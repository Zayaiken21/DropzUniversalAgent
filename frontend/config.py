...
def _candidate_public_config_paths() -> list[Path]:
    paths: list[Path] = []

    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        paths.append(Path(meipass) / "public_config.json")
        paths.append(Path(meipass).parent / "public_config.json")

    paths.extend([
        APP_ROOT / "public_config.json",
        APP_ROOT / "_internal" / "public_config.json",
        Path.cwd() / "public_config.json",
        BASE_DIR.parent / "public_config.json",
        BASE_DIR / "public_config.json",
    ])

    local_appdata = os.getenv("LOCALAPPDATA")
    if local_appdata:
        paths.append(Path(local_appdata) / "DropzUniversalAgent" / "public_config.json")
...
def _public_config() -> dict:
    for path in _candidate_public_config_paths():
        try:
            if path.exists():
...
def _public_config_value(name: str) -> str:
    try:
        return _clean_secret_value(_public_config().get(name, ""))
    except Exception:
        return ""
...
def _secret_or_env(name: str, default: str = "") -> str:
    return (
        ...
        or _env_value(name)
        or (_dotenv_candidates(name)[0] if _dotenv_candidates(name) else "")
        or _public_config_value(name)
        or default
    )
...
def _choose_supabase_key(url: str, name: str) -> str:
    ...
    Priority:
      1. Streamlit secrets / local env / local .env
      2. bundled public_config.json for EXE builds

    public_config.json must only contain public values such as SUPABASE_URL
    and SUPABASE_ANON_KEY. Do not put service role keys in it.
    """
    candidates = _unique([
        _streamlit_secret(name),
        _env_value(name),
        *_dotenv_candidates(name),
        _public_config_value(name),
    ])
    if not candidates:
        return ""
...
# Service role is intentionally NOT read from public_config.json. Keep it out of EXE builds.
SUPABASE_SERVICE_ROLE_KEY = _streamlit_secret("SUPABASE_SERVICE_ROLE_KEY") or _env_value("SUPABASE_SERVICE_ROLE_KEY") or (_dotenv_candidates("SUPABASE_SERVICE_ROLE_KEY")[0] if _dotenv_candidates("SUPABASE_SERVICE_ROLE_KEY") else "")
...
def supabase_config_status() -> dict:
    return {
        ...
        "PUBLIC_CONFIG_FOUND": bool(_public_config()),
    }

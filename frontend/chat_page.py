    ...
    if msg.get("media_path"):
        ...
        if mt.startswith("image/"):
            ...
            with open(media_path, "rb") as f:
                st.download_button("Download attachment", data=f.read(), file_name=Path(media_path).name, key=f"download_{msg['id']}_{idx}")
...
def render_frontend_chat_page():
    ...
    room_id = "main"
    ws_url = st.session_state.get("ws_url", "ws://localhost:8001/ws/chat")
    ...
    if now - st.session_state.chat_last_ping >= 1.0:
    ...
    .block-container {padding-top: .25rem !important; padding-bottom: .25rem !important; max-width: 100% !important;}
    ...
    if st.session_state.chat_more_menu:
        ...
        st.markdown('<div class="members-card">', unsafe_allow_html=True)
        st.markdown("### Active Users")
        for member in active_users:
            st.markdown(f'**{_status_dot(member.get("status","active"))} {member["name"]}**  \n<span class="soft-muted">{member.get("status","active")}</span>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        if send:
            ...
            if msg_text or media_path:
                add_message(user_name=user_name, role=role, message=msg_text, media_path=media_path, media_type=media_type, room_id=room_id)
                rt.send_chat(room_id, user_id, user_name, role, msg_text, media_path, media_type)
                st.session_state.chat_live_edge = True
                st.rerun()

        if voice:
            st.info("Voice note integration is connected through the websocket signal path.")

    st.markdown("</div>", unsafe_allow_html=True)

    components.html("""
    <script>
    const box = window.parent.document.querySelector('.messages-box');

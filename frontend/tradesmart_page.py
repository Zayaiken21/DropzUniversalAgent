...
def _extract_open_trade_tickets(result: Dict[str, Any] | None) -> set[str]:
    """Return currently open trade tickets from a TradeSmart/MT5 snapshot.

    Used to baseline already-open positions when a brand-new agent session
    starts, so old open trades do not get counted as new Session Trades after
    OFF -> ON.
    """
    tickets: set[str] = set()
    ...
    for collection_name in ("position_summary", "positions"):
        rows = result.get(collection_name) or []
...
def _ensure_risk_session(user_key: str, mode: str, enabled: bool) -> str:
    ...
    if enabled and (not was_enabled or was_force_stopped):
        # Brand-new session
        previous_result_key = _scope_key("tradesmart_last_result", user_key, mode)
        previous_result = st.session_state.get(previous_result_key)
        existing_open_tickets = _extract_open_trade_tickets(previous_result)
        ...
        # OFF -> ON must be a clean TradeSmart session. Any positions that were
        # already open before this new session are baselined and excluded from
        # Session Trades / Win-Loss until they are replaced by new agent trades.
        st.session_state[_scope_key("tradesmart_session_baseline_open_tickets", user_key, mode)] = existing_open_tickets
        ...
        st.session_state.pop(previous_result_key, None)
        st.session_state[force_stopped_key] = False
        st.session_state.pop(force_reason_key, None)
        st.session_state.pop(force_key, None)
        _add_log("Risk Session Reset", "New session started. Previous P/L, trades, wins, and losses are the baseline.")
...
def _reset_session_accounting(user_key: str, mode: str, session_id: str) -> None:
    ...
    st.session_state[_scope_key("tradesmart_session_baseline_open_tickets", user_key, mode)] = set()
...
def _apply_session_accounting(result: Dict[str, Any], risk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    baseline_open_tickets_key = _scope_key("tradesmart_session_baseline_open_tickets", user_key, mode)
    ...
    baseline_open_tickets = st.session_state.get(baseline_open_tickets_key)
    if not isinstance(baseline_open_tickets, set):
        baseline_open_tickets = set(baseline_open_tickets or [])

    # If this is the first real snapshot of a brand-new session and there are
    # already open positions, baseline them instead of counting them as new
    # Session Trades. This fixes OFF -> ON showing old open trades in the fresh
    # session trade count/winrate.
    snapshot_open_tickets = _extract_open_trade_tickets(result)
    if not opened_tickets and not baseline_open_tickets and snapshot_open_tickets:
        baseline_open_tickets.update(snapshot_open_tickets)

    for ticket in snapshot_open_tickets:
        if ticket not in baseline_open_tickets:
    ...
    if isinstance(order_result, dict):
        ...
        if ticket not in (None, "") and str(ticket) not in baseline_open_tickets:
            opened_tickets.add(str(ticket))
    ...
    # If this fresh session started with pre-existing open positions, do not let
    # their later close history become this new session's Win/Loss before the
    # agent has opened a new tracked trade in this session.
    if baseline_open_tickets and opened_count <= 0:
        closed_results = []
    ...
    st.session_state[baseline_open_tickets_key] = baseline_open_tickets

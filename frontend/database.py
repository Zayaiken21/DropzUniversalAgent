...
def _supabase_available() -> bool:
    return bool(str(SUPABASE_URL or "").strip() and str(SUPABASE_ANON_KEY or SUPABASE_SERVICE_ROLE_KEY or "").strip())


def _supabase_headers(admin: bool = False) -> Dict[str, str]:
    # Desktop/Streamlit use anon key. service_role is optional private admin/local only.
...
def _supabase_request(method: str, path: str, *, admin: bool = False, **kwargs):
    ...
    if not _supabase_available():
        raise RuntimeError(
            "Supabase is not configured. SUPABASE_URL and SUPABASE_ANON_KEY are missing."
        )
...
def _report_supabase_error(action: str, response=None, exc: Exception | None = None) -> None:
    try:
        if response is not None:
            msg = f"Supabase {action} failed: {response.status_code} {response.text}"
        elif exc is not None:
            msg = f"Supabase {action} error: {exc}"
        else:
...
def _generate_token() -> str:
    # Preserve your short 15-character client-token structure.
...
# ============================================================
# Client token functions — Supabase first, SQLite fallback only
...
def generate_client_token(name, num_tokens=1):
    ...
    # Fallback only for offline/local development.
    conn = get_connection()
    c = conn.cursor()
    for _ in range(int(num_tokens or 1)):
        token = _generate_token()
    ...
    if _supabase_available():
        try:
            path = (
                ...
                f"?token=eq.{token}"
                f"&active=eq.true"
                f"&select=name,token,active,created_at"
            )
...
def cancel_all_client_tokens():
    if _supabase_available():
        try:
            response = _supabase_request(
                "DELETE",
                f"{SUPABASE_TABLE}?token=not.is.null",
                admin=True,
            )
            if response.status_code in (200, 204):
                return
            _report_supabase_error("delete all tokens", response=response)
            return
        except Exception as exc:
            _report_supabase_error("delete all tokens", exc=exc)
            return
